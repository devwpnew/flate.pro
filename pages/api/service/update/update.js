...
import {
    ...
    regenerateProductInfo,
    reGenerateProductName,
...
export default UPDATE = {
    ...
    productsByFilter: async function updateProductsByFilter(
        filter,
        update,
        limit
    ) {
    ...
    regenerateAllProductNames: async function () {
        const getAllProducts = await api.get.product.list({
            limit: "all",
            select: ["id"],
            sort: { id: "ASC" },
        });
    ...
    user: async function updateUser(data) {
        ...
        if (sef_code) {
            ...
            const getCurUser = await api.get.user({
                filter: {
                    id: `${
                        data instanceof FormData ? data.get("id") : data.id
                    }`,
                },
                limit: 1,
            });
    ...
    productViewCount: async function updateProductViewCount(
        product,
        window_host
    ) {
        let getProduct = false;
        if (typeof product == "object") {
            getProduct = await api.get.product.list({
                limit: 1,
        ...
        const updateCountFields = {
            id: getProduct.id,
            table: "product",
            stat_views: getProduct.stat_views
                ? parseInt(getProduct.stat_views + 1)
                : 1,
            date_change: "false",
    ...
    productViewCountPreview: async function updateProductViewCount(
        product,
        window_host
    ) {
        let getProduct = false;
        ...
        const updateCountFields = {
            id: getProduct.id,
            table: "product",
            stat_views_preview: getProduct.stat_views_preview
                ? Number(getProduct.stat_views_preview + 1)
                : 1,
            date_change: "false",
            window_host: window_host,
        };
    ...
    settings: async function updateSettings(fields, host) {
        ...
        if (fields instanceof FormData) {
            for (const pair of fields.entries()) {
                if (pair[0] == "password") {
                    ...
                    resFields.push({
                        name: "master_password_token",
                        value: saltHash.userToken,
                    });
                    resFields.push({
        ...
        return await Promise.all(
            resFields.map(async (field) => {
                ...
                if (!fieldInfo) {
                    return {
                        Error: `Не найдено настройки с именем ${field.name}`,
                    };
                }
