...
import downloadImage from 'public/d-appstore.jpg';
...
export default function GalleryDesktop({ images, srcUrl }) {
                            ...
                            <Gallery>
                                <Item html={`<img src="${srcUrl + src}" />`}>
                                    {({ open }) => (
                                        <>
                                            <div
                                                className="h-[500px] rounded-[10px] overflow-hidden"
                                                onClick={open}
                                            >
                                                <img
    ...
    const galleryThumbs = JSON.parse(images).map((src, index, arr) => {
        return (
            <SwiperSlide key={`${src}${index + 1}`}>
                <div className="border border-transparent transition-all rounded-[10px] overflow-hidden">
                    {!isLoading ? (
                        <Image
    ...
    return (
        <>
            <div className="relative cursor-pointer mb-2.5 rounded-[10px] overflow-hidden">
                <Swiper
                ...
                <div className="absolute left-1/2 -translate-x-1/2 bottom-[15px] z-10 rounded-[10px]">
