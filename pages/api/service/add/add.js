...
export default ADD = {
    ...
    product: async function addProduct(sendData) {
        try {
            if (sendData instanceof FormData) {
                ...
                if (sendData.get("building_address")) {
                    ...
                    if (sendData.get("city_link")) {
                        createBuildingFields.city_link =
                            sendData.get("city_link");
                    }
                    if (sendData.get("area_link")) {
                        createBuildingFields.area_link =
                            sendData.get("area_link");
                    }
            ...
            const isProdModerationOn = await API.get.setting(
                "product_moderation"
            );
            ...
            if (res.full && res.full.itemId) {
                ...
                if (isProdModerationOn == "Y") {
                    API.sendEmailNotification.newProduct(
                        res.full.itemId,
                        domen
                    );
                }
    ...
    personalNotification: async function addPersonalNotification(
        ...
        try {
            ...
            if (!response.error) {
                if (template) {
                    const pushTemplate = pushNotificationTemplates(
                        prod,
                        template
                    );
                    if (pushTemplate) {
                ...
                if (template == "unpublish") {
                    response.EmailUnpub =
                        await API.sendEmailNotification.unpublish(
                            fields.product_id,
                            fields.user_id,
                            domen
                        );
                ...
                if (template == "unprem") {
                    response.EmailUnprem =
                        await API.sendEmailNotification.unprem(
                            fields.product_id,
                            fields.user_id,
                            domen
                        );
    ...
    task: async function addTask(fields) {
        ...
        if (createTaskFields?.status == 1) {
            ...
            const test = await API.sendEmailNotification.feedback(
                createTaskFields
            );
            // console.log({ test })
        }
    ...
    userAdditionalPhone: async function addUserAdditionalPhone(phone, user) {
        ...
        if (arUser.phone == phone) {
            return {
                error: "Данный номер телефона уже используется как главный",
            };
        }
    ...
    pushMessageByToken: async function addPushMessageByToken(
        title,
        text,
        token
    ) {
    ...
    pushMessageByUser: async function addPushMessageByUser(
        title,
        text,
        userid
    ) {
