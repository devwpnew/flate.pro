...
const selectionProductController = {
    ...
    get: async ({ sort, filter, limit, page, select }) => {
        ...
        const query =
            `SELECT ${selectStr} FROM ${selectionProductController.tableName} ${filterStr} ${sortStr} ${limitStr} ${offsetStr}`.trim();
    ...
    getList: async ({ sort, filter, limit, page, select }) => {
        try {
            const res = await selectionProductController.get({
                sort,
                filter,
                limit,
                page,
    ...
    getById: async ({ id, select }) => {
        try {
            return await selectionProductController.get({
                filter: { id },
                select,
                sort: false,
                limit: 1,
    ...
    parseValues: (fields, type = "EDIT") => {
        ...
        if (type == "EDIT") {
            ...
            arValues.push(
                fields.preview_old ? `'${fields.preview_old}'` : "NULL"
            );
            arValues.push(
                fields.gallery ? arrayToDatabase(fields.gallery) : "NULL"
            );
            arValues.push(
                fields.gallery_old
                    ? arrayToDatabase(JSON.parse(fields.gallery_old))
                    : "NULL"
            );
            arValues.push(fields.address ? `'${fields.address}'` : "NULL");
            arValues.push(fields.price ? fields.price : "NULL");
            arValues.push(
                fields.price_for_measure ? fields.price_for_measure : "NULL"
            );
            ...
            arValues.push(
                fields.construction ? `'${fields.construction}'` : "NULL"
            );
            arValues.push(
                fields.communication ? `'${fields.communication}'` : "NULL"
            );
            arValues.push(fields.status ? `'${fields.status}'` : "NULL");
            arValues.push(
                fields.status_lands ? `'${fields.status_lands}'` : "NULL"
            );
        }
    ...
    add: async ({ fields, select }) => {
        ...
        if (!fields.product_id) {
            throw new Error(
                "ID оригинального объекта (fields.product_id) обязательно"
            );
        }

        // Проверка максимального количества объектов в подборке
        const getUserSelectionItemCount =
            await selectionProductController.count({
                filter: { selection: fields.selection },
            });
        if (
            getUserSelectionItemCount?.count &&
            Number(getUserSelectionItemCount.count) >=
                Number(selectionProductController.maxCountForSelection)
        ) {
            throw new Error(
                "Превышено максимальное количество объектов в подборке"
            );
        }

        // Определение значения для сортировки, если оно не указано
        if (!fields.sort) {
            const getMaxSort = await db.any(
                `SELECT id, sort, selection FROM ${selectionProductController.tableName} WHERE selection = $1 ORDER BY sort DESC LIMIT 1`,
                [fields.selection]
            );
        ...
        // Извлечение идентификаторов из объектов
        if (typeof fields.rc === "object" && fields?.rc?.id) {
            fields.rc = fields.rc.id;
        } else if (typeof fields.rc !== "number") {
            fields.rc = null;
            //throw new Error('Поле "rc" должно быть числом.');
        }

        if (typeof fields.type === "object" && fields?.type?.id) {
            fields.type = fields?.type?.id || null;
        } else if (typeof fields.type !== "number") {
            throw new Error('Поле "type" должно быть числом.');
        }

        // Сериализация других объектных полей, если необходимо
        if (typeof fields.sales_contacts === "object") {
            fields.sales_contacts = JSON.stringify(fields.sales_contacts);
        }

        // Убедимся, что поля-массивы являются массивами
        // Поле gallery
        if (typeof fields.gallery === "string") {
            try {
                fields.gallery = JSON.parse(fields.gallery);
            } catch (e) {
                fields.gallery = [];
            }
        }
        ...
        // Поле gallery_old (если оно есть)
        if (typeof fields.gallery_old === "string") {
            try {
                fields.gallery_old = JSON.parse(fields.gallery_old);
            } catch (e) {
                fields.gallery_old = [];
            }
        }
        if (!Array.isArray(fields.gallery_old)) {
            fields.gallery_old = [];
        }

        // Подготовка названий полей и значений для вставки
        const fieldNames = Object.keys(fields);
        const fieldValues = Object.values(fields);

        // Построение параметризованного SQL-запроса
        const query = `
        INSERT INTO ${selectionProductController.tableName} (${fieldNames.join(
            ","
        )})
        VALUES (${fieldNames.map((_, index) => `$${index + 1}`).join(",")})
        RETURNING ${selectToString(select)};
    `;

        try {
            // Выполнение запроса с параметрами
            const request = await db.any(query, fieldValues);
        ...
        // // Преобразуем объекты в JSON, если это необходимо
        // if (typeof fields.gallery === 'object') {
        //     fields.gallery = JSON.stringify(fields.gallery);
        // }

        // if (typeof fields.rc === 'object') {
        //     fields.rc = JSON.stringify(fields.rc);
        // }

        // if (typeof fields.type === 'object') {
    ...
    copyProduct: async ({ productId, selectionId }) => {
        ...
        const selectionInfo = await selectionsController.getById({
            id: selectionId,
        });
        if (!selectionInfo) {
        ...
        if (Array.isArray(properties)) {
            properties.map(
                (prop) =>
                    (prodInfo.properties[prop.prop_code] = prop.prop_value)
            );
        }
        ...
        const newProdFields = {
            ...
            floor_count: prodInfo?.house_floors
                ? prodInfo.house_floors
                : prodInfo?.flat_floors && prodInfo.flat_floors,
            ...
            repair: prodInfo["repairment"]
                ? await api.get.product.fieldPrintableValue(
                      "repairment",
                      prodInfo["repairment"]
                  )
                : "",
            construction: prodInfo["house_construction"]
                ? await api.get.product.fieldPrintableValue(
                      "house_construction",
                      prodInfo["house_construction"]
                  )
                : "",
            communication: prodInfo["house_communication"]
                ? await api.get.product.fieldPrintableValue(
                      "house_communication",
                      prodInfo["house_communication"]
                  )
                : "",
            status: prodInfo["status"]
                ...
                : "",
            status_lands: prodInfo["status_lands"]
                ? await api.get.product.fieldPrintableValue(
                      "status_lands",
                      prodInfo["status_lands"]
                  )
                : "",
        };
        ...
        const selectionProduct = await selectionProductController.add({
    ...
    editItems: async ({ filter, fields, select }) => {
        ...
        const strValues = selectionProductController.parseValues(
            fields,
            "EDIT"
        );
        ...
        try {
            ...
            return controllerError(e, {
                function: "selectionController.edtItems",
                query,
            });
        }
    },
    ...
    editById: async ({ id, fields, select }) => {
        try {
            ...
            const res = await selectionProductController.editItems({
                filter: { id },
                fields,
                select,
            });
            ...
            return controllerError(e, {
                function: "selectionController.editById",
                fields,
                id,
            });
        }
    ...
    updateSort: async ({ selectionProducts }) => {
        ...
        if (!Array.isArray(selectionProducts)) {
            throw new Error(
                "Подборки (selectionProducts) должны быть массивом объектов. Примеры: [{id:1,sort:1},{id:333,sort:2}} или [1,333,555,777]"
            );
        }

        const arResultSelectionProds = selectionProducts.map(
            (selectionProd) => {
        ...
        try {
            const results = await Promise.all(
                arResultSelectionProds.map(async (selectionProdId, index) => {
                    return await selectionProductController.editById({
                        id: selectionProdId,
                        fields: { sort: index + 1 },
                        select: ["id", "sort"],
                    });
            ...
            return controllerError(e, {
                function: "selectionController.updateSort",
                selectionProducts,
            });
        }
    },
    ...
    addProductToSelection: async ({ productId, selectionId }) => {
        try {
            ...
            const newProd = await selectionProductController.copyProduct({
                productId,
                selectionId,
            });
            return newProd;
            ...
            return controllerError(e, {
                function: "selectionController.addproductToSelection",
                productId,
                selectionId,
            });
        }
    ...
    remove: async ({ filter, select }) => {
        ...
        try {
            return db.any(
                `DELETE FROM ${selectionProductController.tableName} ${filterStr}`
            );
        } catch (e) {
            return controllerError(e, {
                function: "selectionController.removeById",
                id,
                select,
            });
        }
    ...
    removeById: async ({ id, select }) => {
        try {
            return db.any(
                `DELETE FROM ${selectionProductController.tableName} WHERE id = ${id}`
            );
        } catch (e) {
            return controllerError(e, {
                function: "selectionController.removeById",
    ...
    count: async ({ filter }) => {
        ...
        try {
            const res = db.one(
                `SELECT COUNT(*) FROM ${selectionProductController.tableName} ${filterStr}`
            );
            return res;
        } catch (e) {
            return controllerError(e, {
                function: "selectionController..count",
                query,
            });
        }
    },
