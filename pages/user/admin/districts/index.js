...
import AdminDistrictsTemplate from "@modules/admin/districts/adminDistrictsTemplate";
...
export default function Districts({ data }) {
  ...
  return (
    <>
      {user && (user.user_group?.id === 1 || user.user_group?.id === 5) ? (
        <div className="min-h-[610px]"><AdminDistrictsTemplate user={user} /></div>
      ) : (
        <Container>
