...
export default function UserEditTemplate({ userId }) {
    ...
    const saveUserRole = async () => {
        ...
        await API.add.pushMessageByUser(
            "Ваш аккаунт одобрен.",
            `Поздравляем! Теперь вам доступен весь функционал FLATE — вашего бизнес-ассистента.`,
            user.id
        );
    ...
    const banUser = async () => {
        ...
        const ban = await API.set.banUser(
            { id: userId },
            false,
            "Бан по причине"
        );
                    ...
                    <Dialog.Panel className="bg-white p-5 max-w-[400px] w-full rounded-[10px]">
                        {isBanLoading ? (
                            ...
                            <DialogMessage
                                ...
                                title={"Вы уверены?"}
                                subtitle={
                                    "Пользователь больше не сможет войти в свой аккаунт"
                                }
