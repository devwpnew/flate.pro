    ...
    try {
        ...
        if (tableColumns) {
            ...
            if (columnsQuery != '' && valuesQuery != '') {
                ...
                // Если создаём новый ЖК, после получения его ID, линкуем ЖК с загруженными видео и удаляем temp_id
                if (table === 'rcs' && fields?.rc_temp_id && res[0].id) {
                    console.log('Начало линковки видео с ЖК');

                    const updateQuery = `
                        UPDATE v2_videos
                        SET rc_id = $1, temp_id = NULL
                        WHERE temp_id = $2
                    `;
                    await db.none(updateQuery, [res[0].id, fields.rc_temp_id]);

                    console.log('Конец линковки видео с ЖК');
                }


                // Если создаём новый оффер, после получения его ID, линкуем оффер с загруженными видео и удаляем temp_id
                if (table === 'product' && fields?.product_temp_id && res[0].id) {
                    console.log('Начало линковки видео с продуктом');

                    const updateQuery = `
                        UPDATE v2_videos
                        SET product_id = $1, temp_id = NULL
                        WHERE temp_id = $2
                    `;
                    await db.none(updateQuery, [res[0].id, fields.product_temp_id]);

                    console.log('Конец линковки видео с продуктом');
                }
